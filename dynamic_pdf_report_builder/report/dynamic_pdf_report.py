...
RENDER_STYLE_SELECTIONS = {
    "layout_style": {"classic", "modern", "minimal"},
    "paper_size": {"a4", "a5"},
    "direction": {"ltr", "rtl"},
    "table_border_style": {"none", "solid", "light"},
}
RENDER_COLOR_FIELDS = {
    "primary_color",
    "secondary_color",
...
class ReportDynamicPdfReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        return self._prepare_dynamic_pdf_rendering_context(
            docids,
            (data or {}).get("report_config"),
        )

    @api.model
    def _prepare_dynamic_pdf_rendering_context(self, docids, report_config):
        """Build the complete custom QWeb contract for every render entry point."""
        if (
            not report_config
        ...
        if len(report_config) != 1:
            raise UserError(_("A Dynamic PDF Report action must resolve to exactly one configuration."))
        ...
        doc_ids = self._normalize_doc_ids(docids)
        ...
        docs = self.env[report_config.model_name].browse(doc_ids)
        ...
        style_config = self._get_render_style_config(report_config)
        style = self._get_style_values(report_config, style_config=style_config)
        if not isinstance(style, dict):
            raise UserError(_("Unable to prepare the Dynamic PDF Report style values."))
        style = {
            key: style.get(key) or ""
            for key in DYNAMIC_PDF_STYLE_KEYS
        }

        return {
            "_dynamic_pdf_report_config_id": report_config.id,
            "doc_ids": doc_ids,
            ...
            "style": style,
            "layout_style": style_config["layout_style"],
            "paper_size": style_config["paper_size"],
            "direction": style_config["direction"],
            "border_color": style_config["border_color"],
            "footer_text": report_config.footer_text or "",
    ...
    @api.model
    def _normalize_doc_ids(self, docids):
        if docids is None:
            return []
        if isinstance(docids, int):
            return [docids]
        if hasattr(docids, "ids"):
            return list(docids.ids)
        return list(docids)
    ...
    @api.model
    def _get_render_style_config(self, report_config):
        style_config = {}
        for field_name, default_value in RENDER_STYLE_DEFAULTS.items():
            value = report_config[field_name] if field_name in report_config._fields else False
            if value in (False, None, ""):
                value = default_value
            if field_name in RENDER_STYLE_SELECTIONS and value not in RENDER_STYLE_SELECTIONS[field_name]:
                value = default_value
            if field_name in RENDER_COLOR_FIELDS and not self._is_render_color(value):
                value = default_value
            style_config[field_name] = value

        style_config["font_size"] = max(int(style_config["font_size"] or 12), 1)
        style_config["title_font_size"] = max(int(style_config["title_font_size"] or 22), 1)
        return style_config

    @api.model
    def _is_render_color(self, value):
        if not isinstance(value, str) or len(value) != 7 or not value.startswith("#"):
            return False
        return all(character in "0123456789abcdefABCDEF" for character in value[1:])

    @api.model
    def _get_style_values(self, report_config, style_config=None):
        style_config = style_config or self._get_render_style_config(report_config)
        text_align = "right" if style_config["direction"] == "rtl" else "left"
        font_size = style_config["font_size"]
        title_font_size = style_config["title_font_size"]
        cell_border = self._get_cell_border_style(style_config)
        header_border = cell_border
        if style_config["table_border_style"] == "none":
            header_border = "border: 0; border-bottom: 1px solid %s;" % style_config["border_color"]

        if style_config["layout_style"] == "modern":
            header_style = (
                "background-color: %s; color: #FFFFFF; padding: 12px 14px; "
                "border-radius: 6px; margin-bottom: 14px;"
            ) % style_config["primary_color"]
            ...
            ) % (style_config["border_color"], style_config["text_color"])
        elif style_config["layout_style"] == "minimal":
            header_style = (
                "border-bottom: 1px solid %s; padding-bottom: 6px; margin-bottom: 12px;"
            ) % style_config["border_color"]
            header_text_style = "color: %s;" % style_config["text_color"]
            table_wrapper_style = ""
            footer_style = (
                "border-top: 1px solid %s; margin-top: 12px; padding-top: 6px; "
                "font-size: 10px; color: %s;"
            ) % (style_config["secondary_color"], style_config["text_color"])
        else:
            header_style = (
        ...
        return {
            ...
            "th": (
                ...
                "background": style_config["table_header_bg_color"],
                "color": style_config["table_header_text_color"],
                "border": header_border,
                "text_align": text_align,
            },
            "td": "%s padding: 6px 8px; text-align: %s; vertical-align: top;" % (cell_border, text_align),
            "zebra_row": "background-color: %s;" % style_config["secondary_color"],
            "footer": footer_style,
    ...
    @api.model
    def _get_cell_border_style(self, style_config):
        if style_config["table_border_style"] == "none":
            return "border: 0;"
        if style_config["table_border_style"] == "light" or style_config["layout_style"] == "minimal":
            return "border: 0; border-bottom: 1px solid %s;" % style_config["border_color"]
        return "border: 1px solid %s;" % style_config["border_color"]
